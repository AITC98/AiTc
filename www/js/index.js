...
var app = {
    ...
    onDeviceReady: function () {
        ...
        $('#terminal form').submit(app.reciveData);
    ...
    listPairedDevices: function () {
        bluetoothSerial.list(function (devices) {
            ...
            devices.forEach(function (device) {
                $list.append('<br /><label><input type="radio" name="device" value="' + device.address +
    ...
    reciveData: function (event) {
        event.preventDefault();

        bluetoothSerial.read(function (x) {

            
            var $dataxxx = $('#data');


            x = '<br /><div class="in">' + x + '</div><br />';
    ...
    reciveData2: function () {

        var xx;
        var y;

        y = bluetoothSerial.read(xx, failure);
    ...
    displayInTerminal: function (data, isIncoming) {
        ...
        if (isIncoming) {
            data = '<br /><span class="in">' + data + '</span><br />';
        }
        else {
            data = '<br /><span class="out">' + data + '</span>';
            data = '<br /><span class="out">' + data + '</span>';
        }
    ...
    ///baray pak sazi etelaaat ferstade shode ya girfteh shodeh
    clearData: function () {
        $('#data').text('');
    },
